...
func (h *CallHandler) readLoop() {
	defer func() {
		if r := recover(); r != nil {
			logError("[%s] recovered in CallHandler.readLoop: %v", h.tag, r)
		}
	}()
	...
	for {
		...
		if h.msgQueue != nil {
			select {
			case h.msgQueue <- text:
			default:
				logError("[%s] Signaling queue full, dropping message", h.tag)
			}
		}
	}
}
...
func (h *CallHandler) signalReconnect() {
	if h.role == "caller" {
		...
		logInfo("[%s] Receiver connection closed, resetting call state", h.tag)
		h.mu.Lock()
		if h.conn != nil {
			h.conn.Close()
			h.conn = nil
...
func (h *CallHandler) sendSDP(sdp string, sdpType string) {
	...
	h.seq++
	logInfo("[%s] Generated SDP %s (%d bytes) [data transport uses ICE-injection]", h.tag, sdpType, len(sdp))
}
...
func startOutgoingCall(client *MaxClient, calleeID int64) *CallHandler {
	...
	h.msgQueue = make(chan string, 128)
	go func() {
		defer func() {
			if r := recover(); r != nil {
				logError("[%s] Recovered from panic in msgQueue: %v", h.tag, r)
			}
		}()
		for text := range h.msgQueue {
			if h.msgHandler != nil {
				h.msgHandler(text)
			}
		}
	...
	h.msgHandler = func(text string) {
			...
			if conv, ok := data["conversation"].(map[string]interface{}); ok {
				if parts, ok := conv["participants"].([]interface{}); ok {
					for _, p := range parts {
						...
						isCreator := false
						for _, r := range roles {
							if rStr, ok := r.(string); ok && rStr == "CREATOR" {
								isCreator = true
							}
						}
						if !isCreator {
							if idF, ok := part["id"].(float64); ok {
								h.localID = int64(idF)
		...
		if c, ok := d["candidate"].(map[string]interface{}); ok {
			if useICEInjection {
				candidateStr, _ := c["candidate"].(string)
				decode, err := base64.StdEncoding.DecodeString(candidateStr)
...
func startIncomingListener(client *MaxClient) *CallHandler {
	...
	h.msgHandler = func(text string) {
		...
		if sdp, ok := d["sdp"].(map[string]interface{}); ok {
			sdpType, _ := sdp["type"].(string)
			sdpData, _ := sdp["sdp"].(string)
			if sdpType != "" && sdpData != "" {
				h.handleSDP(sdpType, sdpData)
			}
			return
		}
		if c, ok := d["candidate"].(map[string]interface{}); ok {
