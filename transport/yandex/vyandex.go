...
func (r *relayClient) Start() {
	for i := 0; i < r.workers; i++ {
		r.wg.Add(1)
		workerID := i
		utils.SafeGo(fmt.Sprintf("volga.worker-%d", workerID), func() {
			r.worker(workerID)
		})
	}
...
func (w *wsListener) Start() {
	utils.SafeGo("volga.wsListener", w.run)
}
...
func (t *YandexVolgaTransport) Start() error {
	...
	utils.SafeGo("volga.keepAliveLoop", t.keepAliveLoop)
	utils.SafeGo("volga.statsLoop", t.statsLoop)
	t.SetConnected(true)
